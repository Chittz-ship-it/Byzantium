...
// Weights must sum to 100. "nothing" entries return no item.
const rollLootTable = (table) => {
...
const isSellable = (name) => !SPECIAL[name];
const hasSeal    = (player) => player.inventory.some((i) => i.name === "Merchant Seal");
const sellValue  = (player, v) => hasSeal(player) ? Math.floor(v * 1.1) : v;
...
// ── ENEMIES — all loot tables sum to 100 ─────────────────
const ENEMIES = {
  rabbit: {
    id: "rabbit", name: "Wild Rabbit",
    art: ["  (\\(\\   ", "  ( •ᴗ•) ", "  o(\")(\")"],
    color: "#a8e6cf",
    maxHp: 18, atk: 4, def: 0, xp: 12,
    loot: [
      { name: "Rabbit Pelt",    weight: 50, value: 12 },
      { name: "Tiny Bones",     weight: 20, value: 4  },
      { name: "Silver Earring", weight: 2,  value: 45 },
  ...
  vole: {
    ...
    art: ["   ,--,  ", "  (·ω·)  ", "   mm mm "],
    color: "#d4b896",
    maxHp: 25, atk: 7, def: 1, xp: 18,
    loot: [
      { name: "Vole Pelt",      weight: 50, value: 8  },
      { name: "Mini Bones",     weight: 20, value: 3  },
      { name: "Bronze Monocle", weight: 2,  value: 30 },
      { name: "nothing",        weight: 28             },
    ],
    weight: 24,
  },
  fox: {
    ...
    art: ["  /^\\/^  ", " ( °ᴥ° ) ", "  )    ( "],
    color: "#f97316",
    maxHp: 45, atk: 12, def: 4, xp: 45,
    loot: [
      { name: "Fox Pelt",    weight: 50, value: 18 },
      { name: "Small Bones", weight: 20, value: 5  },
      { name: "Copper Ring", weight: 2,  value: 35 },
      { name: "nothing",     weight: 28             },
    ],
    weight: 24,
  },
  peasant: {
    id: "peasant", name: "Rabid Peasant",
    art: ["   \\O/   ", "    |    ", "   / \\  "],
    color: "#86efac",
    maxHp: 35, atk: 11, def: 2, xp: 32,
    loot: [
      { name: "Loincloth",    weight: 50, value: 8 },
      { name: "Warm Ale",     weight: 20, value: 0 },
      { name: "Sack of Coin", weight: 2,  value: 0 },
      { name: "nothing",      weight: 28            },
    ],
    weight: 24,
  },
  purplefox: {
    id: "purplefox", name: "Purple Fox",
    art: ["  /^\\/^  ", " ( ×ᴥ× ) ", "  )~✦~( "],
    color: "#c084fc",
    maxHp: 75, atk: 20, def: 8, xp: 90,
    loot: [
      { name: "Purple Fox Pelt", weight: 50, value: 55 },
      { name: "Magic Bones",     weight: 20, value: 28 },
      { name: "Amethyst",        weight: 2,  value: 75 },
      { name: "nothing",         weight: 28             },
    ],
    weight: 1,
  },
  parrot: {
    id: "parrot", name: "Wild Parrot",
    art: ["   _/\\_  ", "  (o o)  ", " <=|U|=> "],
    color: "#4ade80",
    maxHp: 28, atk: 9, def: 2, xp: 25,
    loot: [
      { name: "Red Feather",    weight: 20, value: 10 },
      { name: "Blue Feather",   weight: 20, value: 10 },
      { name: "Green Feather",  weight: 20, value: 10 },
      { name: "Orange Feather", weight: 20, value: 10 },
      { name: "Purple Feather", weight: 20, value: 18 },
    ],
    weight: 1,
  },
  boar: {
    id: "boar", name: "Charging Boar",
    art: ["  C====3  ", " (@oo@)  ", "  mm  mm "],
    color: "#92400e",
    maxHp: 90, atk: 25, def: 9, xp: 110,
    loot: [
      { name: "Boar Hide",      weight: 50, value: 40 },
      { name: "Boar Bones",     weight: 20, value: 20 },
      { name: "Crown of Tusks", weight: 2,  value: 90 },
      { name: "nothing",        weight: 28             },
    ],
    weight: 1,
  },
  noble: {
    id: "noble", name: "Fighting Noble",
    art: ["  [≡≡≡]  ", " (⌐■_■)  ", "  /| |\\  "],
    color: "#fbbf24",
    maxHp: 70, atk: 22, def: 11, xp: 130,
    loot: [
      { name: "Sack of Coin",       weight: 50, value: 0 },
      { name: "Large Sack of Coin", weight: 20, value: 0 },
      { name: "Merchant Seal",      weight: 2,  value: 0 },
      { name: "nothing",            weight: 28            },
    ],
    weight: 1,
  },
...
const MAX_LEVEL = 10;
const getLevel  = (xp) => { for (let i = MAX_LEVEL; i >= 1; i--) if (xp >= XP_CURVE[i]) return i; return 1; };
...
const Tab = ({ label, active, onClick }) => (
  <button onClick={onClick} style={{
    flex: 1, fontFamily: "'VT323', monospace", fontSize: "0.9rem",
...
const TitleScreen = ({ onStart }) => (
  <div style={{ textAlign: "center", padding: "28px 20px 24px" }}>
    <div style={{
      color: T.gold, fontFamily: "'VT323', monospace",
      fontSize: "clamp(2.2rem, 11vw, 3rem)", letterSpacing: "0.12em",
      lineHeight: 1, marginBottom: 2,
      textShadow: `0 0 30px rgba(212,168,67,0.45)`,
    }}>
      BYZANTIUM
    </div>
...
🌲🌲🌲🌲🌲🌲🌲🌲`}
    ...
    <div style={{ marginBottom: 18 }}>
      <Btn onClick={onStart} variant="primary" style={{ fontSize: "1.3rem", padding: "12px 40px", width: "100%" }}>
        ▶  BEGIN ADVENTURE
      </Btn>
    </div>

    <div style={{
      background: "#070f09", border: `1px solid ${T.border}`, borderRadius: 4,
      padding: "10px 14px", textAlign: "left",
    }}>
      <div style={{ color: T.dim, fontFamily: "'VT323', monospace", fontSize: "0.75rem", marginBottom: 6, letterSpacing: "0.1em" }}>
        ZONE I · THE FOREST
      </div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: "4px 12px" }}>
        {Object.values(ENEMIES).map((e) => (
          <span key={e.id} style={{ color: e.color, fontFamily: "'VT323', monospace", fontSize: "0.8rem" }}>
            {e.name}
          </span>
        ))}
      </div>
    ...
    <div style={{ color: "#1a3320", fontFamily: "'VT323', monospace", fontSize: "0.72rem", marginTop: 14 }}>
      v2.0 · Hunt · Loot · Conquer
    </div>
  </div>
);
...
const NameScreen = ({ onConfirm }) => {
  ...
  const startStats = calcStats(1, DEFAULT_EQUIP);

  return (
    <div style={{ padding: "24px 20px" }}>
      <div style={{ color: T.gold, fontFamily: "'VT323', monospace", fontSize: "1.4rem", textAlign: "center", marginBottom: 2 }}>
        WHO ARE YOU, TRAVELLER?
      </div>
      <div style={{ color: T.dim, fontFamily: "'VT323', monospace", fontSize: "0.88rem", textAlign: "center", marginBottom: 20 }}>
      ...
      <input
        ...
        style={{
          ...
          color: T.green, fontFamily: "'VT323', monospace", fontSize: "1.3rem",
          padding: "10px 16px", borderRadius: 3, width: "100%",
      ...
      <Panel style={{ marginBottom: 16 }}>
        <div style={{ color: T.amber, fontFamily: "'VT323', monospace", fontSize: "0.82rem", marginBottom: 10, letterSpacing: "0.1em", textAlign: "center" }}>
          ── STARTING CHARACTER ──
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
          {[["❤ HP", startStats.maxHp], ["⚔ ATK", startStats.atk], ["🛡 DEF", startStats.def]].map(([label, val]) => (
            <div key={label} style={{
              background: "#060e08", border: `1px solid ${T.border}`,
              borderRadius: 3, padding: "8px 4px", textAlign: "center",
            }}>
              <div style={{ color: T.dim,   fontFamily: "'VT323', monospace", fontSize: "0.72rem" }}>{label}</div>
              <div style={{ color: T.white, fontFamily: "'VT323', monospace", fontSize: "1.2rem"  }}>{val}</div>
            </div>
          ))}
        </div>
        <Divider />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 10px" }}>
          {[
            ["⚔ Weapon",  SHOP.weapons[0].name],
            ["🛡 Armour",  SHOP.armour[0].name ],
            ["💍 Ring",    "None"              ],
            ["💎 Earring", "None"              ],
            ["💰 Gold",    "10g"               ],
            ["★ Level",   "1"                 ],
          ].map(([label, val]) => (
            <div key={label} style={{ fontFamily: "'VT323', monospace", fontSize: "0.82rem" }}>
      ...
      <Btn onClick={() => clean && onConfirm(clean)} variant="primary" disabled={!clean}
        style={{ fontSize: "1.15rem", padding: "11px 28px", width: "100%" }}>
        ENTER THE WORLD →
      </Btn>
    </div>
  );
  ...
  ━━━━━━━━━━━━━━━━━━━━━━`}
      ...
      {player.inventory.length > 0 && (
        <div style={{
          ...
          background: "#1f1500", border: `1px solid ${T.gold}`, borderRadius: 3,
          color: T.gold, fontFamily: "'VT323', monospace", fontSize: "0.9rem", textAlign: "center",
        }}>
...
// ── SCREEN: COMBAT ────────────────────────────────────────
const CombatScreen = ({ player, stats, enemy, log, phase, onAttack, onFlee, onReturn, onHuntAgain, onUseAle }) => {
  const [showItems, setShowItems] = useState(false);
  const aleCount = player.inventory.filter((i) => i.name === "Warm Ale").length;
    ...
    <div style={{ padding: 16 }}>
      ...
      {showItems && phase === "player_turn" && (
        <Panel style={{ marginTop: 8, padding: 10 }}>
          <div style={{ color: T.amber, fontFamily: "'VT323', monospace", fontSize: "0.85rem", marginBottom: 8 }}>USE ITEM</div>
          {aleCount === 0 ? (
            <div style={{ color: T.dim, fontFamily: "'VT323', monospace", fontSize: "0.85rem" }}>No usable items.</div>
          ) : (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                <div style={{ color: T.amber, fontFamily: "'VT323', monospace", fontSize: "0.9rem" }}>
                  🍺 Warm Ale <span style={{ color: T.dim }}>×{aleCount}</span>
                </div>
                <div style={{ color: T.dim, fontFamily: "'VT323', monospace", fontSize: "0.75rem" }}>Heals 15–25 HP</div>
              </div>
              <Btn onClick={() => { onUseAle(); setShowItems(false); }} variant="amber" style={{ padding: "4px 10px", fontSize: "0.85rem" }}>
                Drink
              </Btn>
            </div>
          )}
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
        {phase === "player_turn" && (
          <>
            ...
            <Btn onClick={() => setShowItems((s) => !s)} variant="amber" disabled={aleCount === 0} style={{ flex: 1 }}>
              🍺 {aleCount > 0 ? `Ale (${aleCount})` : "No Items"}
            </Btn>
...
const ShopScreen = ({ player, onSell, onSellAll, onBuy, onOpenSack, onBack }) => {
  ...
  const sealActive   = hasSeal(player);
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <span style={{ color: T.text, fontFamily: "'VT323', monospace", fontSize: "0.9rem" }}>
          Your Gold {sealActive && <span style={{ color: T.gold }}>· 🔖 +10%</span>}
        </span>
        ...
        {tab === "sell" && (
          <>
            {lootItems.length === 0 ? (
              ...
              <>
                {lootItems.map((item) => {
                  const sv = sellValue(player, item.value);
                  return (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 4, marginBottom: 12 }}>
              {Object.keys(SHOP).map((cat) => (
                <button key={cat} onClick={() => setBuyTab(cat)} style={{
                  ...
                  padding: "5px 2px", borderRadius: 3, cursor: "pointer", textTransform: "capitalize",
                }}>
        ...
        {tab === "items" && (
          <>
            {specialItems.length === 0 ? (
              <div style={{ color: T.dim, fontFamily: "'VT323', monospace", textAlign: "center", padding: "20px 0" }}>
                No special items in your bag.
              </div>
            ) : specialItems.map((item) => {
              const sp   = SPECIAL[item.name];
              const type = sp?.type;
              return (
                <div key={item.name} style={{
                  display: "flex", justifyContent: "space-between", alignItems: "center",
                  ...
                  <div>
                    <div style={{
                      color: type === "key" ? T.gold : type === "usable" ? T.amber : T.blue,
                      fontFamily: "'VT323', monospace", fontSize: "0.95rem",
                    }}>
                      {type === "key" ? "🔖 " : type === "usable" ? "🍺 " : "💰 "}
                      {item.name}
                      {item.count > 1 && <span style={{ color: T.dim }}> ×{item.count}</span>}
                    </div>
                    <div style={{ color: T.dim, fontFamily: "'VT323', monospace", fontSize: "0.75rem" }}>
                      {type === "key"      && sp.desc}
                      {type === "usable"   && `Heals ${sp.healMin}–${sp.healMax} HP · Use in combat`}
                      {type === "openable" && `Contains ${sp.goldMin}–${sp.goldMax}g`}
                    </div>
                  </div>
                  {type === "openable" && (
                    <Btn onClick={() => onOpenSack(item.name)} variant="gold" style={{ padding: "4px 10px", fontSize: "0.85rem" }}>
                      Open
                    </Btn>
                  )}
                  {type === "key" && (
                    <span style={{ color: T.gold, fontFamily: "'VT323', monospace", fontSize: "0.78rem" }}>ACTIVE</span>
                  )}
                  {type === "usable" && (
                    <span style={{ color: T.dim, fontFamily: "'VT323', monospace", fontSize: "0.75rem" }}>Combat only</span>
                  )}
                </div>
              );
            })}
...
export default function App() {
  ...
  const handleAttack = () => {
    ...
    if (newEHP <= 0) {
      const drops  = rollLootTable(enemy.loot);
  ...
  const handleUseAle = () => {
    if (combatPhase !== "player_turn") return;
    const aleIdx = player.inventory.findIndex((i) => i.name === "Warm Ale");
    if (aleIdx === -1) return;
    const sp    = SPECIAL["Warm Ale"];
    const heal  = rand(sp.healMin, sp.healMax);
    const inv   = [...player.inventory];
    inv.splice(aleIdx, 1);
    const healed = Math.min(player.hp + heal, stats.maxHp);
    const eDmg  = Math.max(1, enemy.atk + rand(-2, 3) - stats.def);
    const newHP = Math.max(0, healed - eDmg);
    const newLog = [
      `🍺 You drink the Warm Ale and recover ${heal} HP!`,
      `🐾 The ${enemy.name} attacks for ${eDmg}!`,
    ];
    if (newHP <= 0) {
      newLog.push(`💀 You fall unconscious...`);
      newLog.push(`You wake in Sylvantide, battered.`);
      setPlayer((p) => ({ ...p, hp: Math.max(1, Math.floor(stats.maxHp * 0.3)), inventory: inv }));
      setCombatLog((prev) => [...prev, ...newLog]);
      setCombatPhase("defeat");
    } else {
  ...
  const handleSellAll = () =>
    setPlayer((p) => {
      ...
      const earned = loot.reduce((s, i) => s + sellValue(p, i.value), 0);
      return { ...p, gold: p.gold + earned, inventory: others };
    });

  const handleBuy = (item) =>
    setPlayer((p) => {
      if (p.gold < item.cost) return p;
      const newEquip = { ...p.equipment, [item.slot]: item };
      const newStats = calcStats(getLevel(p.xp), newEquip);
      return { ...p, gold: p.gold - item.cost, equipment: newEquip, hp: Math.min(p.hp, newStats.maxHp) };
    });

  const handleOpenSack = (sackName) =>
    setPlayer((p) => {
      const idx = p.inventory.findIndex((i) => i.name === sackName);
      if (idx === -1) return p;
      const sp  = SPECIAL[sackName];
      const g   = rand(sp.goldMin, sp.goldMax);
      const inv = [...p.inventory];
      inv.splice(idx, 1);
      ...
      <div style={{
        ...
        {screen === "combat" && player && enemy && (
          <CombatScreen player={player} stats={stats} enemy={enemy}
            ...
            onAttack={handleAttack} onFlee={handleFlee} onUseAle={handleUseAle}
        ...
        {screen === "shop"  && player && (
          <ShopScreen player={player}
            onSell={handleSell} onSellAll={handleSellAll}
            onBuy={handleBuy}   onOpenSack={handleOpenSack}
