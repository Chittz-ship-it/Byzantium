...
// ─────────────────────────────────────────────────────────
// WORKER URL — paste your Cloudflare Worker URL here
// e.g. "https://matthewbrindley.charliechitty.workers.dev"
// ─────────────────────────────────────────────────────────
const WORKER_URL = "https://matthewbrindley.charliechitty.workers.dev";

// ── API CALL — all errors surface as thrown strings ───────
const callAI = async (messages) => {
  ...
  try {
    ...
    throw `Network error reaching Worker: ${e.message}`;
  }
  ...
  try {
    ...
    throw `Worker returned non-JSON (status ${res.status})`;
  }

  if (!res.ok) {
    throw `Worker/NVIDIA error ${res.status}: ${data.error || ""} ${data.nvidia_response ? "— NVIDIA said: " + data.nvidia_response.slice(0, 200) : ""}`;
  }

  const content = data.choices?.[0]?.message?.content;
  if (!content) throw `NVIDIA response had no content: ${JSON.stringify(data).slice(0, 200)}`;
  return content;
};
...
const parseJSON = (text) => {
  try {
    ...
    if (s === Infinity) throw new Error("No JSON found");
...
// ── SAVE / LOAD ───────────────────────────────────────────
const SAVE_KEY  = "byzantium_save_v3";
const saveGame  = (player, screen, huntZone, currentAiZone) => {
  try {
    localStorage.setItem(SAVE_KEY, JSON.stringify({
      player,
      screen: ["combat","ai_loading","zone_unlocked","talent_selection"].includes(screen) ? "town" : screen,
      huntZone,
      currentAiZone,
      savedAt: Date.now(),
    }));
...
const loadGame  = () => { try { const s = localStorage.getItem(SAVE_KEY); return s ? JSON.parse(s) : null; } catch { return null; } };
...
const rollLoot = (table, flags = {}) => {
  ...
  for (const e of table) {
    ...
    if (r < cum) {
      ...
      return [{ name: e.name, value: typeof e.value === "function" ? e.value() : (e.value ?? 0) }];
    }
  }
  return [];
};
...
const fixLoot = (loot) => {
  if (!loot?.length) return [{ name: "nothing", weight: 100 }];
...
const SPECIAL = {
  ...
  "Talent Chit":             { type:"talent",  desc:"Choose a permanent talent · Tap Use" },
  "Churchyard Map":          { type:"map",     desc:"Generates The Churchyard zone via AI" },
  "Swamp Map":               { type:"map",     desc:"Generates The Swamp zone via AI" },
  "Spiritworld Map":         { type:"map",     desc:"Generates The Spiritworld zone via AI" },
  "Book: Solid Strikes V1":  { type:"book",    effect:"solidStrikes",  desc:"+1% crit chance · Crits deal double damage" },
  "Book: Iron Will":         { type:"book",    effect:"ironWill",      desc:"Permanently +5 DEF" },
  "Book: Battle Hymn":       { type:"book",    effect:"battleHymn",    desc:"Permanently +5 ATK" },
  "Book: Swift Feet":        { type:"book",    effect:"swiftFeet",     desc:"+15% flee success rate" },
  "Book: Expansive Mind":    { type:"book",    effect:"expandedMind",  desc:"Raises level cap from 10 to 15" },
...
const getHolyDmg   = (p) => (p.holyPermDmg || 0) + (Date.now() < (p.holyExpiry || 0) ? 10 : 0);
...
const calcStats = (lvl, eq=DEFAULT_EQUIP) => {
  ...
  for (const g of Object.values(eq).filter(Boolean)) {
    if(g.atk) b.atk+=g.atk; if(g.def) b.def+=g.def; if(g.hp) b.maxHp+=g.hp;
  }
  return b;
};
...
const MOUNTAIN_ENEMIES = {
  ...
  merchant:{id:"merchant",name:"Travelling Merchant",art:["  [~~~~]  "," (^‿^)   ","  |___|  "],color:"#fbbf24",special:"merchant",maxHp:1,atk:0,def:0,xp:0,weight:1,loot:[]},
  ...
  priest:  {id:"priest",  name:"Calm Priest",        art:["   †   †  "," (^ω^)   ","   |   |  "],color:"#bfdbfe",special:"priest",maxHp:1,atk:0,def:0,xp:0,weight:1,loot:[]},
};
...
const mkPlayer = (name) => {
  ...
  return {
    ...
    maxLevel:10, critChance:0, flatAtk:0, flatDef:0,
    fleeBonus:0, holyPermDmg:0, holyExpiry:0,
    learnedTalents:[], unlockedZones:[], zoneCount:0,
    flags:{ solidStrikesDropped:false, expandedMindLearned:false,
...
const RECIPES = [
  {
    ...
    name:"Headdress", flavor:"Woven from five enchanted feathers.", outputDesc:"Key Item · +1% evasion",
    ingredients:[
    ...
    forge:p=>{
      let inv=[...p.inventory];
      for(const f of["Red Feather","Blue Feather","Green Feather","Orange Feather","Purple Feather"]){const x=inv.findIndex(i=>i.name===f);if(x!==-1)inv.splice(x,1);}
      return{...p,inventory:[...inv,{name:"Headdress",value:0}]};
    },
  },
  {
    id:"shadowfang_plus", requiresRecipe:null,
    name:"Shadowfang ★", flavor:"Imbued with purple fox essence.", outputDesc:"Weapon · +42 ATK",
    ingredients:[
      {label:"Shadowfang (equipped weapon)",has:p=>p.equipment.weapon?.id==="shadowfang"},
    ...
    alreadyHave:p=>["shadowfang_plus","shadowfang_star"].includes(p.equipment.weapon?.id),
    forge:p=>{
      const u={id:"shadowfang_plus",name:"Shadowfang ★",slot:"weapon",cost:0,atk:42};
      let inv=[...p.inventory];const x=inv.findIndex(i=>i.name==="Purple Fox Pelt");if(x!==-1)inv.splice(x,1);
      return{...p,inventory:inv,equipment:{...p.equipment,weapon:u}};
    },
  },
  {
    id:"warlords_cloak", requiresRecipe:null,
    name:"Warlord's Cloak", flavor:"Shadow Cloak reinforced with boar hide.", outputDesc:"Armour · +18 DEF · +80 HP",
    ingredients:[
      {label:"Shadow Cloak (equipped armour)",has:p=>p.equipment.armour?.id==="shadow"},
    ...
    forge:p=>{
      const u={id:"warlords_cloak",name:"Warlord's Cloak",slot:"armour",cost:0,def:18,hp:80};
      let inv=[...p.inventory];const x=inv.findIndex(i=>i.name==="Boar Hide");if(x!==-1)inv.splice(x,1);
      const eq={...p.equipment,armour:u};
  ...
  },
  {
    ...
    name:"King's Amethyst Seal", flavor:"The King's Seal set with an amethyst.", outputDesc:"Ring · +8 ATK · +6 DEF · +25 HP",
    ingredients:[
      {label:"King's Seal (equipped ring)",has:p=>p.equipment.ring?.id==="kings_seal"},
    ...
    forge:p=>{
      const u={id:"amethyst_seal",name:"King's Amethyst Seal",slot:"ring",cost:0,atk:8,def:6,hp:25};
      let inv=[...p.inventory];const x=inv.findIndex(i=>i.name==="Amethyst");if(x!==-1)inv.splice(x,1);
      const eq={...p.equipment,ring:u};
      return{...p,inventory:inv,equipment:eq,hp:Math.min(p.hp,calcStats(getLevel(p.xp,p.maxLevel),eq).maxHp)};
    },
  },
  {
    id:"void_shard_plus", requiresRecipe:null,
    name:"Void Shard ★", flavor:"The Void Shard threaded with silver.", outputDesc:"Earring · +12 ATK · +5 DEF · +22 HP",
    ingredients:[
      {label:"Void Shard (equipped earring)",has:p=>p.equipment.earring?.id==="void_shard"},
    ...
    forge:p=>{
      const u={id:"void_shard_plus",name:"Void Shard ★",slot:"earring",cost:0,atk:12,def:5,hp:22};
      let inv=[...p.inventory];const x=inv.findIndex(i=>i.name==="Silver Earring");if(x!==-1)inv.splice(x,1);
      const eq={...p.equipment,earring:u};
      return{...p,inventory:inv,equipment:eq,hp:Math.min(p.hp,calcStats(getLevel(p.xp,p.maxLevel),eq).maxHp)};
    },
  },
  {
    id:"shadowfang_alt", requiresRecipe:"Recipe for Shadowfang ★",
    name:"Shadowfang ★ (Alt)", flavor:"Five bone types fused into the blade.", outputDesc:"Weapon · +42 ATK",
    ingredients:[
      {label:"Shadowfang (equipped weapon)",has:p=>p.equipment.weapon?.id==="shadowfang"},
    ...
    alreadyHave:p=>["shadowfang_plus","shadowfang_star"].includes(p.equipment.weapon?.id),
    forge:p=>{
      const u={id:"shadowfang_star",name:"Shadowfang ★",slot:"weapon",cost:0,atk:42};
      let inv=[...p.inventory];
      for(const b of["Tiny Bones","Mini Bones","Small Bones","Boar Bones","Magic Bones"]){const x=inv.findIndex(i=>i.name===b);if(x!==-1)inv.splice(x,1);}
      return{...p,inventory:inv,equipment:{...p.equipment,weapon:u}};
    },
  },
];
...
const generateZone = async (mapName, level, atk, def, zoneCount=0) => {
  const cfg = ZONE_CFG[mapName] || {theme:"Unknown Lands",flavor:"strange creatures",hpMult:1.5,atkMult:1.5};
  const isFinalBoss = zoneCount >= 20;
  const scale = Math.pow(1.18, zoneCount);
  const cHp  = Math.round(150 * cfg.hpMult * scale);
  const cAtk = Math.round(40  * cfg.atkMult * scale);
  const cDef = Math.round(12  * cfg.hpMult * scale);
  const cXp  = Math.round(200 * cfg.hpMult * scale);
  const tier  = zoneCount < 5 ? "early" : zoneCount < 10 ? "mid" : zoneCount < 15 ? "hard" : "brutal";

  const prompt = isFinalBoss
    ? `You are a game designer for dark fantasy RPG Byzantium. Generate THE FINAL BOSS.
Player: Lv${level} ATK${atk} DEF${def}. This is the end of the game.
Return ONLY raw JSON, starting immediately with {
{"zoneName":"The Final Reckoning","description":"One sentence of absolute dread.","campDescription":"There is no camp. Only this.","isFinalBoss":true,"enemies":[{"id":"final_boss","name":"THE SOVEREIGN DARKNESS","art":["  (art1)  ","  (art2)  ","  (art3)  "],"color":"#ff0000","maxHp":${Math.round(cHp*6)},"atk":${Math.round(cAtk*2.5)},"def":${Math.round(cDef*2)},"xp":${cXp*10},"weight":1,"loot":[{"name":"Crown of the Fallen","weight":40,"value":5000},{"name":"Soul Gem","weight":30,"value":3000},{"name":"Ancient Relic","weight":28,"value":2000},{"name":"nothing","weight":2}]}]}`
    : `You are a game designer for dark fantasy RPG Byzantium. Generate zone "${cfg.theme}" iteration ${zoneCount+1} (${tier} difficulty).
Theme: ${cfg.flavor}. Player: Lv${level} ATK${atk} DEF${def}.
Return ONLY raw JSON, starting immediately with {
{"zoneName":"${cfg.theme} — ${tier.charAt(0).toUpperCase()+tier.slice(1)} Reaches","description":"One dark sentence.","campDescription":"One sentence about the camp.","enemies":[{"id":"unique_id","name":"Enemy Name","art":["  (art)  ","  (art)  ","  (art)  "],"color":"#hex","maxHp":${cHp},"atk":${cAtk},"def":${cDef},"xp":${cXp},"weight":24,"loot":[{"name":"Drop Name","weight":50,"value":${Math.round(60*scale)}},{"name":"Drop2","weight":20,"value":${Math.round(30*scale)}},{"name":"Rare Drop","weight":2,"value":${Math.round(180*scale)}},{"name":"nothing","weight":28}]}]}
RULES: exactly 4 enemies weight:24 and 2 enemies weight:1. Every loot array sums to 100. Art arrays exactly 3 strings ~9 chars. All IDs unique snake_case.`;

  const raw  = await callAI([{role:"user", content:prompt}]);
  const data = parseJSON(raw);
  if (!data || !Array.isArray(data.enemies) || data.enemies.length < (isFinalBoss ? 1 : 4)) {
    throw `Zone JSON invalid — got: ${raw.slice(0,300)}`;
  }
  data.id        = `${mapName.replace(" Map","").toLowerCase().replace(/\s+/g,"_")}_${zoneCount}`;
  data.mapName   = mapName;
  data.zoneCount = zoneCount;
  data.isFinalBoss = isFinalBoss;
  data.enemies   = data.enemies.map(e => ({...e, hp:e.maxHp, loot:fixLoot(e.loot)}));
  return data;
};

const generateTalents = async (player, level) => {
  const prompt = `Generate 3 distinct permanent talent choices for a dark fantasy RPG character.
Player: Lv${level}, Crit ${Math.round((player.critChance||0)*100)}%, BonusATK ${player.flatAtk||0}, BonusDEF ${player.flatDef||0}.
Return ONLY a raw JSON array starting with [
[{"id":"talent_id","name":"Talent Name","description":"Clear mechanical benefit","effect":"atk","value":6}]
RULES: exactly 3 talents. Valid effects: "atk"(4-10),"def"(3-8),"hp"(25-60),"crit"(0.01-0.03),"flee"(0.10-0.20),"holy_perm"(5-15). No two share an effect. Dark fantasy names.`;
  const raw    = await callAI([{role:"user", content:prompt}]);
  const parsed = parseJSON(raw);
  if (!Array.isArray(parsed) || parsed.length < 3) throw `Talent JSON invalid — got: ${raw.slice(0,200)}`;
...
const buildMerchantPool = (player) => {
  const pool = [];
  const wId=player.equipment.weapon?.id, aId=player.equipment.armour?.id,
        rId=player.equipment.ring?.id,   eId=player.equipment.earring?.id;
  if(!["shadowfang","shadowfang_plus","shadowfang_star"].includes(wId)) pool.push({id:"r_sf",name:"Recipe: Shadowfang",cost:200,type:"recipe",desc:"Forge Shadowfang · Forest Blade + 3 Fox Pelts"});
  ...
  const wT=["stick","dagger","sword","blade"].indexOf(wId??"stick");
  const aT=["rags","leather","chain","plate"].indexOf(aId??"rags");
  ...
  if(!rId||rId==="copper_ring") pool.push({id:"g_ks",name:"King's Seal",cost:500,type:"gear",item:SHOP.rings[4],desc:"+5 ATK · +4 DEF · +20 HP"});
  if(!eId||eId==="bone_hook")   pool.push({id:"g_vs",name:"Void Shard",cost:580,type:"gear",item:SHOP.earrings[4],desc:"+8 ATK · +3 DEF · +15 HP"});
  return pool;
};
const genOffer = (player) => { const p=buildMerchantPool(player); return p.length ? p[Math.floor(Math.random()*p.length)] : null; };
...
const CombatLog=({lines})=>{
  ...
  const col=l=>{
    ...
    if(l.includes("💀")||l.includes("unconscious")||l.includes("retaliates")||l.includes("attacks"))return T.red;
  ...
  return(
    <Panel style={{marginBottom:12}}>
      ...
      <div style={{display:"flex",gap:10,marginTop:6,flexWrap:"wrap"}}>
        ...
        {(player.critChance||0)>0&&<span style={{color:T.gold,fontFamily:"'VT323',monospace",fontSize:"0.72rem"}}>⚡ {Math.round(player.critChance*100)}% Crit</span>}
        {holy>0&&<span style={{color:T.blue,fontFamily:"'VT323',monospace",fontSize:"0.72rem"}}>☩ +{holy} Holy</span>}
        {(player.fleeBonus||0)>0&&<span style={{color:T.green,fontFamily:"'VT323',monospace",fontSize:"0.72rem"}}>🏃 +{Math.round(player.fleeBonus*100)}%</span>}
      </div>
    </Panel>
  );
};
...
const TitleScreen=({onStart,onContinue,saveData})=>(
  <div style={{textAlign:"center",padding:"28px 20px 24px"}}>
    ...
    {saveData&&(
      <div style={{marginBottom:10}}>
        ...
        <div style={{color:T.dim,fontFamily:"'VT323',monospace",fontSize:"0.78rem",marginTop:6}}>
          {saveData.player?.name}  ·  Lv{getLevel(saveData.player?.xp||0,saveData.player?.maxLevel||10)}  ·  {saveData.player?.gold}g
        </div>
      </div>
    )}
    <div style={{marginBottom:14}}>
      <Btn onClick={onStart} variant={saveData?"default":"primary"} style={{fontSize:saveData?"1rem":"1.3rem",padding:"10px 40px",width:"100%"}}>
        {saveData?"▶  NEW GAME":"▶  BEGIN ADVENTURE"}
      </Btn>
    </div>
    ...
    <div style={{background:"#070f09",border:"1px solid #1a2a3a",borderRadius:4,padding:"10px 14px",textAlign:"left",marginBottom:8}}>
      <div style={{color:"#2a4a6a",fontFamily:"'VT323',monospace",fontSize:"0.75rem",marginBottom:4,letterSpacing:"0.1em"}}>ZONE II · THE MOUNTAIN  [Lv10 · Map required]</div>
    ...
    <div style={{background:"#070f09",border:"1px solid #1a1a2a",borderRadius:4,padding:"10px 14px",textAlign:"left"}}>
      <div style={{color:"#2a2a5a",fontFamily:"'VT323',monospace",fontSize:"0.75rem",marginBottom:4,letterSpacing:"0.1em"}}>ZONES III+ · AI GENERATED  [Map required]</div>
      <div style={{color:"#1a1a3a",fontFamily:"'VT323',monospace",fontSize:"0.8rem"}}>The Churchyard · The Swamp · The Spiritworld</div>
    </div>
    <div style={{color:"#1a3320",fontFamily:"'VT323',monospace",fontSize:"0.72rem",marginTop:14}}>v3.1 · Hunt · Loot · Conquer · AI Worlds</div>
  </div>
);
...
// ── SCREEN: AI LOADING ────────────────────────────────────
const LoadingScreen=({message,error,onRetry,onCancel})=>(
  <div style={{padding:"50px 20px",textAlign:"center"}}>
    {!error&&<div style={{color:T.gold,fontFamily:"'VT323',monospace",fontSize:"2.5rem",marginBottom:16,animation:"pulse 1.5s ease-in-out infinite"}}>⚙</div>}
    {error&&<div style={{color:T.red,fontFamily:"'VT323',monospace",fontSize:"2rem",marginBottom:16}}>✗</div>}
    <div style={{color:error?T.red:T.amber,fontFamily:"'VT323',monospace",fontSize:"1.2rem",marginBottom:12}}>{error?"ERROR":"GENERATING..."}</div>
    <div style={{color:T.dim,fontFamily:"'VT323',monospace",fontSize:"0.85rem",maxWidth:300,margin:"0 auto",lineHeight:1.6,wordBreak:"break-word",marginBottom:20}}>
      {error||message}
    </div>
    {error&&(
      <div style={{display:"flex",gap:8,justifyContent:"center"}}>
        <Btn onClick={onRetry} variant="primary">Try Again</Btn>
...
const ZoneUnlockedScreen=({zone,onEnter,onLater})=>(
  <div style={{padding:20}}>
    <div style={{color:zone.isFinalBoss?T.red:T.gold,fontFamily:"'VT323',monospace",fontSize:"1.4rem",textAlign:"center",marginBottom:4,textShadow:zone.isFinalBoss?"0 0 20px rgba(248,113,113,0.6)":"none"}}>
      {zone.isFinalBoss?"⚠ THE FINAL RECKONING ⚠":"✦ ZONE UNLOCKED ✦"}
    </div>
    <div style={{color:T.dim,fontFamily:"'VT323',monospace",fontSize:"0.85rem",textAlign:"center",marginBottom:16,letterSpacing:"0.1em"}}>{zone.zoneName.toUpperCase()}</div>
    <Panel style={{marginBottom:14,border:`1px solid ${zone.isFinalBoss?T.red:T.border}`}}>
      <div style={{color:T.text,fontFamily:"'VT323',monospace",fontSize:"0.95rem",lineHeight:1.7,marginBottom:10}}>{zone.description}</div>
      {zone.isFinalBoss&&zone.enemies[0]&&(
        <>
          <pre style={{color:T.red,fontFamily:"monospace",fontSize:"1.1rem",lineHeight:1.5,margin:"8px 0"}}>{zone.enemies[0].art.join("\n")}</pre>
          <div style={{color:T.red,fontFamily:"'VT323',monospace",fontSize:"1.1rem"}}>{zone.enemies[0].name}</div>
          <div style={{color:T.dim,fontFamily:"'VT323',monospace",fontSize:"0.82rem"}}>HP {zone.enemies[0].maxHp} · ATK {zone.enemies[0].atk} · DEF {zone.enemies[0].def}</div>
        </>
      )}
      {!zone.isFinalBoss&&(
        <>
          <div style={{color:T.dim,fontFamily:"'VT323',monospace",fontSize:"0.82rem",marginBottom:10}}>{zone.campDescription}</div>
    ...
    <div style={{display:"flex",gap:8}}>
      <Btn onClick={onEnter} variant={zone.isFinalBoss?"danger":"primary"} style={{flex:1,padding:12}}>
        {zone.isFinalBoss?"⚔ Face Your Fate":"⚔ Enter Now"}
      </Btn>
      <Btn onClick={onLater} style={{flex:1,padding:12}}>← Later</Btn>
    </div>
  </div>
);
    ...
    <div style={{padding:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
        ...
        <Btn onClick={mapOwned?onMountain:null} disabled={!mapOwned} variant={mapOwned?"steel":"dim"} style={{padding:12,fontSize:"1rem",gridColumn:"1 / -1"}}>
          ⛰ {mapOwned?"Enter The Mountain":"Mountain  (Map required · Lv10)"}
        </Btn>
      </div>
...
const AiZoneCampScreen=({zone,player,level,stats,onHunt,onStats,onRest,onReturn})=>{
  ...
  return(
    <div style={{padding:16}}>
      ...
      <PlayerPanel player={player} level={level} stats={stats}/>
      <Panel style={{marginBottom:12,padding:"8px 14px"}}>
  ...
  return(
    <div style={{padding:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
        <Panel style={{padding:10}}>
          ...
          <div style={{display:"flex",gap:6,marginTop:4,flexWrap:"wrap"}}>
            ...
            {(player.critChance||0)>0&&<span style={{color:T.gold,fontFamily:"'VT323',monospace",fontSize:"0.65rem"}}>⚡</span>}
            {getHolyDmg(player)>0&&<span style={{color:T.blue,fontFamily:"'VT323',monospace",fontSize:"0.65rem"}}>☩</span>}
          </div>
        </Panel>
        <Panel style={{padding:10}}>
      ...
      <Panel style={{textAlign:"center",marginBottom:10,padding:"12px 10px"}}>
        <pre style={{color:enemy.hp>0?(enemy.color||T.text):T.dim,fontFamily:"monospace",fontSize:"1.15rem",lineHeight:1.5,margin:0,opacity:enemy.hp<=0?0.3:1,transition:"opacity 0.4s"}}>
          {(enemy.art||["  ???  ","  ???  ","  ???  "]).join("\n")}
        </pre>
          ...
          {totalHeal===0?<div style={{color:T.dim,fontFamily:"'VT323',monospace",fontSize:"0.85rem"}}>No usable items.</div>:(
            <>
              {ales.length>0&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                <div><div style={{color:T.amber,fontFamily:"'VT323',monospace"}}>🍺 Warm Ale ×{ales.length}</div><div style={{color:T.dim,fontFamily:"'VT323',monospace",fontSize:"0.72rem"}}>Heals 15–25 HP</div></div>
              ...
              {milks.length>0&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div><div style={{color:T.blue,fontFamily:"'VT323',monospace"}}>🥛 Warm Milk ×{milks.length}</div><div style={{color:T.dim,fontFamily:"'VT323',monospace",fontSize:"0.72rem"}}>Heals 30–45 HP</div></div>
...
const MerchantEncounterScreen=({player,stats,offer,onBuy,onLeave})=>(
  <div style={{padding:16}}>
    ...
    <div style={{color:T.dim,fontFamily:"'VT323',monospace",fontSize:"0.85rem",marginBottom:14}}>"One item, one price. Take it or leave it."</div>
    {offer?(
      <Panel style={{marginBottom:14,border:`1px solid ${T.gold}`}}>
        ...
        <Btn onClick={()=>player.gold>=offer.cost&&onBuy(offer)} disabled={player.gold<offer.cost} variant="gold" style={{width:"100%",fontSize:"1.05rem",padding:10}}>
          {player.gold>=offer.cost?"Buy":"Not enough gold"}
        </Btn>
      </Panel>
    ):(
      <Panel style={{marginBottom:14,textAlign:"center",padding:"20px 14px"}}><div style={{color:T.dim,fontFamily:"'VT323',monospace"}}>"I'm fresh out of things you'd want."</div></Panel>
    )}
...
const PriestEncounterScreen=({player,stats,onDonate,onLeave})=>{
  ...
  return(
    <div style={{padding:16}}>
      ...
      <div style={{color:T.dim,fontFamily:"'VT323',monospace",fontSize:"0.85rem",marginBottom:14}}>He sits peacefully by a small stone altar.</div>
              ...
              return(
                <div key={item.name} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${T.border}`}}>
                  ...
                  <div style={{flexShrink:0,marginLeft:8}}>
                    ...
                    {type==="map"&&<Btn onClick={()=>onUseMap(item.name)} variant="purple" style={{padding:"4px 10px",fontSize:"0.85rem"}}>{(player.zoneCount||0)>=20?"⚠ Final Boss":"Use Map"}</Btn>}
                    {type==="talent"&&<Btn onClick={()=>onUseTalentChit()} variant="amber" style={{padding:"4px 10px",fontSize:"0.85rem"}}>Use Chit</Btn>}
...
const BlacksmithScreen=({player,onForge,onBack})=>{
  ...
  return(
    <div style={{padding:16}}>
      ...
      {visible.map(r=>{const already=r.alreadyHave(player);const can=!already&&r.canCraft(player);return(
        <Panel key={r.id} style={{marginBottom:10,border:`1px solid ${already?T.greenDim:can?T.amber:T.border}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
            <div><div style={{color:already?T.green:can?T.amber:T.text,fontFamily:"'VT323',monospace",fontSize:"1.05rem"}}>{already?"✓ ":""}{r.name}</div><div style={{color:T.purple,fontFamily:"'VT323',monospace",fontSize:"0.78rem"}}>{r.outputDesc}</div></div>
            {already?<span style={{color:T.green,fontFamily:"'VT323',monospace",fontSize:"0.78rem"}}>FORGED</span>:<Btn onClick={()=>can&&onForge(r.id)} disabled={!can} variant={can?"amber":"dim"} style={{padding:"4px 10px",fontSize:"0.85rem"}}>Forge</Btn>}
          </div>
          <div style={{color:T.dim,fontFamily:"'VT323',monospace",fontSize:"0.75rem",marginBottom:8,fontStyle:"italic"}}>{r.flavor}</div>
          <div style={{borderTop:`1px solid ${T.border}`,paddingTop:8}}>
            {r.ingredients.map(ing=>{const have=ing.has(player);return<div key={ing.label} style={{color:have?T.green:T.red,fontFamily:"'VT323',monospace",fontSize:"0.82rem",marginBottom:2}}>{have?"✓":"✗"} {ing.label}</div>;})}
          </div>
        </Panel>
      );})}
      ...
      <Btn onClick={onBack} style={{width:"100%",marginTop:4}}>← Back to Camp</Btn>
    </div>
  );
};
    ...
    <div style={{padding:16}}>
      ...
      <Panel style={{marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:10,marginTop:8,flexWrap:"wrap"}}>
          ...
          {(player.critChance||0)>0&&<span style={{color:T.gold,fontFamily:"'VT323',monospace",fontSize:"0.78rem"}}>⚡ {Math.round(player.critChance*100)}% crit</span>}
          {holy>0&&<span style={{color:T.blue,fontFamily:"'VT323',monospace",fontSize:"0.78rem"}}>☩ +{holy} holy</span>}
          {(player.fleeBonus||0)>0&&<span style={{color:T.green,fontFamily:"'VT323',monospace",fontSize:"0.78rem"}}>🏃 +{Math.round(player.fleeBonus*100)}% flee</span>}
        </div>
        {player.learnedTalents?.length>0&&<><Divider label="TALENTS"/>{player.learnedTalents.map(t=><div key={t.id} style={{color:T.amber,fontFamily:"'VT323',monospace",fontSize:"0.82rem",marginBottom:2}}>⚡ {t.name}</div>)}</>}
        <Divider/>
        <div style={{color:T.dim,fontFamily:"'VT323',monospace",fontSize:"0.78rem",marginBottom:4}}>
          {level<player.maxLevel?`Progress: ${player.xp-XP_CURVE[level]} / ${xpForNext(level,player.maxLevel)-XP_CURVE[level]} XP`:"Maximum level achieved."}
        </div>
        <XpBar xp={player.xp} level={level} maxLevel={player.maxLevel}/>
      </Panel>
      <Panel style={{marginBottom:12}}>
...
export default function App() {
  ...
  const bStats  = player ? calcStats(level, player.equipment) : calcStats(1);
  const stats   = player ? {...bStats, atk:bStats.atk+(player.flatAtk||0), def:bStats.def+(player.flatDef||0)} : bStats;

  // Auto-save whenever player changes
  useEffect(()=>{ if(player) saveGame(player,screen,huntZone,currentAiZone); },[player]);

  const handleContinue=()=>{
    if(!saveData)return;
    setPlayer(saveData.player); setCurrentAiZone(saveData.currentAiZone||null);
    setHuntZone(saveData.huntZone||"forest"); setScreen(saveData.screen||"town"); setSaveData(null);
  };
  const handleNewGame=()=>{ deleteSave(); setSaveData(null); setScreen("name"); };
  ...
  // ── COMBAT ────────────────────────────────────────────────
  const doEvasion=(currentEnemy,currentStats)=>{
    if(checkEvade(player))return{dmg:0,msg:`✦ ${currentEnemy.name}'s attack misses! (Headdress)`};
    const dmg=Math.max(1,currentEnemy.atk+rand(-2,3)-currentStats.def);
    return{dmg,msg:`🐾 The ${currentEnemy.name} retaliates for ${dmg}!`};
  };
  ...
  const startFight=(e,zone,aiZone=null)=>{
    ...
    setCombatLog([`You venture into ${zone==="mountain"?"the mountain pass":aiZone?aiZone.zoneName:"the shadowed forest"}...`,`A ${e.name} appears!`]);
    setCombatPhase("player_turn"); setScreen("combat");
  };

  const handleAttack=()=>{
    if(combatPhase!=="player_turn")return;
    const log=[]; const holy=getHolyDmg(player);
    const isCrit=(player.critChance||0)>0&&Math.random()<player.critChance;
    let pDmg=Math.max(1,stats.atk+rand(-2,4)-enemy.def)+holy;
    if(isCrit)pDmg=Math.floor(pDmg*2);
    log.push(`⚔ You strike for ${pDmg}!${isCrit?" ⚡ CRITICAL HIT!":""}${holy?` (+${holy} holy)`:""}`);
    const newEHP=Math.max(0,enemy.hp-pDmg);

    if(newEHP<=0){
      const drops=rollLoot(enemy.loot||[],player.flags||{});
      ...
      log.push(`💀 The ${enemy.name} collapses!`);
      log.push(`✨ +${enemy.xp} XP!`);
      if(drops.length)log.push(`📦 Loot: ${drops.map(d=>d.name).join(", ")}`);
      if(lvUp)log.push(`🌟 LEVEL UP! Now Level ${newLvl}!`);
      setEnemy(e=>({...e,hp:0}));
      ...
      setCombatLog(prev=>[...prev,...log]); setCombatPhase("victory"); return;
    }

    const{dmg,msg}=doEvasion({...enemy,hp:newEHP},stats);
    log.push(msg);
    const newPHP=Math.max(0,player.hp-dmg);
    if(newPHP<=0){
      log.push(`💀 You fall unconscious...`); log.push(`You wake, battered.`);
      setEnemy(e=>({...e,hp:newEHP}));
      setPlayer(p=>({...p,hp:Math.max(1,Math.floor(stats.maxHp*0.3))}));
      setCombatLog(prev=>[...prev,...log]); setCombatPhase("defeat"); return;
    }
    setEnemy(e=>({...e,hp:newEHP})); setPlayer(p=>({...p,hp:newPHP}));
  ...
  const handleFlee=()=>{
    ...
    else{ const{dmg,msg}=doEvasion(enemy,stats); setPlayer(p=>({...p,hp:Math.max(1,p.hp-dmg)})); setCombatLog(prev=>[...prev,`🏃 Failed! ${msg}`]); }
  };
  ...
  const handleSellAll=()=>setPlayer(p=>{
    const loot=p.inventory.filter(i=>isSellable(i.name));
    const others=p.inventory.filter(i=>!isSellable(i.name));
  ...
  const handleOpenSack=(sackName)=>setPlayer(p=>{
    ...
    const sp=SPECIAL[sackName]; const g=rand(sp.goldMin,sp.goldMax);
  ...
  const handleUseMap=async(mapName)=>{
    ...
    setPendingMap(mapName); setAiError(""); setAiMsg(`The AI is building ${ZONE_CFG[mapName]?.theme||mapName.replace(" Map","")}...\nThis takes 15–30 seconds.`);
    setScreen("ai_loading");
    try{
      const zone=await generateZone(mapName,level,stats.atk,stats.def,player.zoneCount||0);
      setPlayer(p=>({...p,unlockedZones:[...(p.unlockedZones||[]),zone],zoneCount:(p.zoneCount||0)+1}));
      setNewZone(zone); setPendingMap(null); setScreen("zone_unlocked");
    }catch(err){
      setAiError(`Failed to generate zone.\n\nError: ${err}\n\nYour map has been returned.`);
      setPlayer(p=>({...p,inventory:[...p.inventory,{name:mapName,value:0}]}));
    }
  };

  const handleRetryMap=()=>{ if(pendingMap) handleUseMap(pendingMap); };
  const handleCancelMap=()=>{ setPendingMap(null); setAiError(""); setScreen("town"); };
  ...
  const handleUseTalentChit=async()=>{
    ...
    setAiError(""); setAiMsg("The AI is forging your destiny...\nChoose wisely. This is permanent.");
    setScreen("ai_loading");
    try{
      const talents=await generateTalents(player,level);
      setPendingTalents(talents); setScreen("talent_selection");
    }catch(err){
      setAiError(`Failed to generate talents.\n\nError: ${err}\n\nYour Talent Chit has been returned.`);
  ...
  const handleMerchantBuy=(offer)=>{
    ...
    setScreen(huntZone==="mountain"?"mountain_camp":currentAiZone?"ai_zone_camp":"town");
  };
  const handlePriestDonate=()=>{
    setPlayer(p=>{
  ...
  const returnFromCombat=()=>setScreen(huntZone==="mountain"?"mountain_camp":currentAiZone?"ai_zone_camp":"town");
  const huntAgain=()=>{
    if(huntZone==="mountain")startFight(spawnMountain(),"mountain");
    else if(huntZone==="ai_zone"&&currentAiZone)startFight(spawnAiZone(currentAiZone),"ai_zone",currentAiZone);
      ...
      <div style={{width:"100%",maxWidth:420,background:T.bg,border:`1px solid ${T.border}`,borderRadius:6,boxShadow:"0 0 60px rgba(74,222,128,0.06)",overflow:"hidden",wordBreak:"break-word"}}>
        ...
        <div style={{background:T.panel,borderBottom:`1px solid ${T.border}`,padding:"7px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          {player&&<span style={{color:T.dim,fontFamily:"'VT323',monospace",fontSize:"0.82rem"}}>
            ...
            {(player.critChance||0)>0&&<span style={{color:T.gold}}> · ⚡</span>}
            {getHolyDmg(player)>0&&<span style={{color:T.blue}}> · ☩</span>}
          </span>}
        </div>
        ...
        {screen==="ai_loading"&&<LoadingScreen message={aiMsg} error={aiError} onRetry={pendingMap?handleRetryMap:handleUseTalentChit} onCancel={pendingMap?handleCancelMap:handleCancelTalent}/>}
        {screen==="zone_unlocked"&&newZone&&<ZoneUnlockedScreen zone={newZone}
          onEnter={()=>{
            if(newZone.isFinalBoss){ setCurrentAiZone(newZone); setNewZone(null); startFight(newZone.enemies[0],"ai_zone",newZone); }
            else{ setCurrentAiZone(newZone); setNewZone(null); setScreen("ai_zone_camp"); }
          }}
          onLater={()=>{setNewZone(null);setScreen("town");}}
        />}
        {screen==="talent_selection"&&<TalentSelectionScreen talents={pendingTalents} onPick={handlePickTalent} onCancel={handleCancelTalent}/>}

        {screen==="town"&&player&&<TownScreen player={player} level={level} stats={stats}
        ...
        />}
        {screen==="mountain_camp"&&player&&<MountainCampScreen player={player} level={level} stats={stats}
        ...
        />}
        {screen==="ai_zone_camp"&&player&&currentAiZone&&<AiZoneCampScreen zone={currentAiZone} player={player} level={level} stats={stats}
          onHunt={()=>startFight(spawnAiZone(currentAiZone),"ai_zone",currentAiZone)}
          onStats={()=>goStats("ai_zone_camp")}
          onRest={doRest}
          onReturn={()=>setScreen("town")}
        />}
        {screen==="combat"&&player&&enemy&&<CombatScreen player={player} stats={stats} enemy={enemy}
          log={combatLog} phase={combatPhase}
          onAttack={handleAttack} onFlee={handleFlee} onUseItem={handleUseItem}
          onReturn={returnFromCombat} onHuntAgain={huntAgain}
        />}
        {screen==="merchant_encounter"&&player&&<MerchantEncounterScreen player={player} stats={stats} offer={merchantOffer}
          onBuy={handleMerchantBuy}
          onLeave={()=>setScreen(huntZone==="mountain"?"mountain_camp":currentAiZone?"ai_zone_camp":"town")}
        />}
        {screen==="priest_encounter"&&player&&<PriestEncounterScreen player={player} stats={stats}
          onDonate={handlePriestDonate}
          onLeave={()=>setScreen(huntZone==="mountain"?"mountain_camp":"town")}
        />}
        {screen==="shop"&&player&&<ShopScreen player={player} level={level}
          onSell={handleSell} onSellAll={handleSellAll} onBuy={handleBuy}
          onOpenSack={handleOpenSack} onReadBook={handleReadBook} onBuyMap={handleBuyMap}
